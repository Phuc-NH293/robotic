      ...
      <div className="page">
        ...
        {!stats && !error ? <Loading /> : error ? <div className="error-state">{error}</div> : stats && <>
          <section className="metrics">
            ...
            <article className="metric-card"><div className="metric-top"><span className="metric-icon green">✓</span><em>+2.1%</em></div><p>Tỷ lệ đạt</p><h2>{stats.pass_rate}%</h2><div className="metric-line"><span style={{ width: `${stats.pass_rate}%` }} /></div><small>{stats.passed_count.toLocaleString("vi-VN")} sản phẩm đạt chuẩn</small></article>
            <article className="metric-card"><div className="metric-top"><span className="metric-icon red">!</span><em className="bad">−8.3%</em></div><p>Lỗi phát hiện</p><h2>{stats.defect_count}</h2><div className="defect-types"><span>Nứt <b>{stats.defect_distribution["Nứt / Rạn"] || 0}</b></span><span>Bề mặt <b>{stats.defect_distribution["Bề mặt"] || 0}</b></span></div></article>
          ...
          <section className="dashboard-grid">
            ...
            <article className="panel live-panel">
              <div className="panel-head"><div><p className="eyebrow">TRỰC TIẾP</p><h3>Robot Cell 01</h3></div><span className="live-badge"><i /> LIVE</span></div>
              <div className="camera-feed"><div className="feed-grid" /><div className="machine-part"><span /><i /></div><div className="detect-box"><span>WELD JOINT · 98%</span></div><div className="feed-top"><span>CAM-01 / MAIN</span><span>14:32:08</span></div><div className="feed-bottom"><span>ISO 5817 · LEVEL B</span><span>FRAME 2840</span></div><div className="sweep" /></div>
          ...
          <section className="bottom-grid">
            <article className="panel trend-panel"><div className="panel-head"><div><p className="eyebrow">XU HƯỚNG 7 NGÀY</p><h3>Chất lượng sản xuất</h3></div><div className="legend"><span><i className="pass" />Đạt</span><span><i className="fail" />Không đạt</span></div></div><div className="chart">{[[90,7],[88,10],[94,4],[91,8],[96,3],[93,5],[98,2]].map((v,i) => <div className="chart-day" key={i}><i className="c-pass" style={{ height:`${v[0]}%` }} /><i className="c-fail" style={{ height:`${Math.max(v[1],5)}%` }} /><span>{["T2","T3","T4","T5","T6","T7","CN"][i]}</span></div>)}</div></article>
            <article className="panel defect-panel"><div className="panel-head"><div><p className="eyebrow">PHÂN BỐ</p><h3>Loại lỗi phổ biến</h3></div></div><div className="donut-wrap"><div className="donut"><div><strong>{stats.defect_count}</strong><span>TỔNG LỖI</span></div></div><div className="donut-legend"><p><i className="d1" /><span>Nứt / Rạn</span><strong>45%</strong></p><p><i className="d2" /><span>Lỗi bề mặt</span><strong>35%</strong></p><p><i className="d3" /><span>Sai vị trí</span><strong>12%</strong></p><p><i className="d4" /><span>Khác</span><strong>8%</strong></p></div></div></article>
          </section>
        </>}
      </div>
    </AppShell>
