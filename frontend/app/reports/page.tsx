...
export default function ReportsPage() {
  ...
  function exportCsv() {
    const rows = [["Mã báo cáo","Hạng mục tường","Thời gian","Robot","Kết quả","Số lỗi","Người duyệt"], ...filtered.map((r) => [r.id,r.product,r.inspected_at,r.robot,r.status,r.defect_count,r.reviewer])];
    ...
    <AppShell title="Báo cáo kiểm tra">
      <div className="page">
        ...
        <section className="report-toolbar">
          <div className="search-box"><span>⌕</span><input placeholder="Tìm mã báo cáo, hạng mục tường..." value={search} onChange={(e) => setSearch(e.target.value)} /></div>
          ...
          <select value={product} onChange={(e) => setProduct(e.target.value)}><option value="all">Tất cả hạng mục</option><option>Tường phòng khách A102</option><option>Tường hành lang tầng 2</option><option>Cột bê tông sảnh chính</option><option>Trần thạch cao sảnh phụ</option></select>
        </section>
        <section className="panel report-table-panel">
          <div className="table-summary"><span><b>{filtered.length}</b> báo cáo</span><span>Cập nhật từ máy chủ</span></div>
          {loading ? <Loading /> : <div className="table-scroll"><table><thead><tr><th>MÃ BÁO CÁO</th><th>HẠNG MỤC TƯỜNG</th><th>THỜI GIAN</th><th>ROBOT</th><th>KẾT QUẢ</th><th>LỖI</th><th>NGƯỜI DUYỆT</th><th /></tr></thead><tbody>
